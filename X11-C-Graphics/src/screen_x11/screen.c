...
static Atom getSupportedAtom(screen_x11_t *screen_x11, Atom *supportedAtoms,
		unsigned long atomCount, const char *atomName);

static unsigned long getWindowProperty(screen_x11_t *screen_x11, Window window,
		Atom property, Atom type, unsigned char **value);
...
//    screen_start_time = nano_time();
	...
	int count;
	XPixmapFormatValues *pixmap_formats = XListPixmapFormats(screen->display,
			&count);
	for (int i = 0; i != count; i++) {
		if (pixmap_formats[i].depth == default_depth) {
			screen->depth = (uint32_t) pixmap_formats[i].depth;
			screen->bytespp = (uint32_t) (pixmap_formats[i].bits_per_pixel
					/ BITS_PER_BYTE);
...
//    screen->overrideRedirect = true;
	...
	{
		XVisualInfo vi;
		int result = XMatchVisualInfo(screen->display, screen->screen_idx,
				(int32_t) screen->depth, TrueColor, &vi);
		if (result) {
			printf(
					"Visual is TrueColor, %d bytes per pix, %d bytes per rgb_init\n",
					screen->bytespp, vi.depth / BITS_PER_BYTE);
			//                sinfo = new l3d_screen_info_rgb(vis->red_mask, vis->green_mask, vis->blue_mask, bytespp, vi.default_depth / BITS_PER_BYTE);
		}
		else {
			fprintf(stderr,
					"couldn't get visual information, XMatchVisualInfo\n");
			exit(EXIT_FAILURE);
		}
	}
		break;
	...
	XSetForeground(screen->display, screen->gc,
			WhitePixel(screen->display, screen->screen_idx));
	screen->screen_info_rgb = init_screen_info(screen->vis->red_mask,
			screen->vis->green_mask, screen->vis->blue_mask, screen->bytespp,
			screen->depth / BITS_PER_BYTE, NULL);
...
static void set_override_redirect(screen_x11_t *screen)
{
	...
	XChangeWindowAttributes(screen->display, screen->window,
...
static void get_cursor_pos(screen_x11_t *screen, int *windowX, int *windowY)
{
	...
	int rootX, rootY;
	unsigned int mask;
	XQueryPointer(screen->display, screen->window, &root, &window, &rootX,
			&rootY, /* Screen coords relative to root window's top-left */
...
static void move_mouse_root_win(screen_x11_t *screen)
{
	...
	XTranslateCoordinates(screen->display, screen->window, screen->root,
			screen->width / 2, screen->height / 2, &x, &y, &child);
	printf("%d %d\n", x, y);
//        XFlush(screen->display);
...
{
	XShmPutImage(screen->display, screen->window, screen->gc, screen->ximg, 0,
			0, 0, 0, (unsigned int) screen->width,
...
static void print_coord_root(screen_t *screen)
{
	Window child;
	int x, y;
	XTranslateCoordinates(screen->display, screen->window, screen->root, 0, 0,
			&x, &y, &child);
...
static XImage* create_image(screen_x11_t *screen_x11)
{
	XImage *ximg = XShmCreateImage(screen_x11->display, screen_x11->vis,
			(unsigned int) screen_x11->depth,
			ZPixmap,   // format
			0,  // pixels to ignore at beg. of scanline
			&screen_x11->shminfo, (unsigned int) screen_x11->width,
			(unsigned int) screen_x11->height);
	screen_x11->shminfo.shmid = shmget(IPC_PRIVATE,
			(size_t) (ximg->bytes_per_line * ximg->height), IPC_CREAT | 0777);
	if (screen_x11->shminfo.shmid < 0) {
		...
		screen_x11->shminfo.shmaddr = ximg->data = (char*) shmat(
				screen_x11->shminfo.shmid, 0, 0);
...
static void init_atoms(screen_x11_t *screen_x11)
{
	...
	screen_x11->wmDeleteWindow = XInternAtom(screen_x11->display,
			"WM_DELETE_WINDOW",
			False);
	Atom wmSupported = XInternAtom(screen_x11->display, "_NET_SUPPORTED",
	True);
	// Now we need to check the _NET_SUPPORTED property of the root window
	atomCount = getWindowProperty(screen_x11, screen_x11->root, wmSupported,
...
//    set_video_mode(screen, screen->width, screen->height); // already done in set win size?
	...
	XResizeWindow(screen->display, screen->window, screen->width,
			screen->height);
	XSync(screen->display, False);
		...
		Atom property, Atom type, unsigned char **value)
{
	...
	XGetWindowProperty(screen_x11->display, window, property, 0,
	LONG_MAX,
	False, type, &actualType, &actualFormat, &itemCount, &bytesAfter, value);
	if (actualType != type) {
		return 0;
	}
	return itemCount;
...
static void change_video_mode(screen_x11_t *screen, int mode)
{
	...
	// Get a list of all available display modes
	XF86VidModeGetAllModeLines(screen->display, screen_idx, &modecount,
			&modelist);
...
static void restore_video_mode(screen_x11_t *screen)
{
	if (screen->FS.modeChanged) {
		...
		XF86VidModeSwitchToMode(screen->display, screen->screen_idx,
				&screen->FS.oldMode);
		screen->FS.modeChanged = false;
	}
}
...
static Cursor create_null_cursor(Display *display, Window root)
{
	...
	col.red = 0;
	col.flags = 4;
	cursor = XCreatePixmapCursor(display, cursormask, cursormask, &col, &col, 0,
			0);
...
{ // Grab cursor to user window
	if (!screen->pointer_grabbed) {
		...
		PointerMotionMask, GrabModeAsync, GrabModeAsync, screen->window, None,
		CurrentTime);
...
static bool check_for_EWMH(screen_x11_t *screen)
{
	...
	Atom supportingWmCheck = XInternAtom(screen->display,
			...
			True);
	Atom wmSupported = XInternAtom(screen->display, "_NET_SUPPORTED",
	True);
	...
	if (getWindowProperty(screen, screen->root, supportingWmCheck,
	XA_WINDOW, (unsigned char**) &windowFromRoot) != 1) {
	...
	if (getWindowProperty(screen, *windowFromRoot, supportingWmCheck,
	XA_WINDOW, (unsigned char**) &windowFromChild) != 1) {
		XFree(windowFromRoot);
	...
	// Now we need to check the _NET_SUPPORTED property of the root window
	atomCount = getWindowProperty(screen, screen->root, wmSupported,
	XA_ATOM, (unsigned char**) &supportedAtoms);

	// See which of the atoms we support that are supported by the WM

	screen->wmState = getSupportedAtom(screen, supportedAtoms, atomCount,
			"_NET_WM_STATE");
	screen->wmStateFullscreen = getSupportedAtom(screen, supportedAtoms,
			atomCount, "_NET_WM_STATE_FULLSCREEN");
	screen->wmPing = getSupportedAtom(screen, supportedAtoms, atomCount,
			"_NET_WM_PING");
	screen->wmActiveWindow = getSupportedAtom(screen, supportedAtoms, atomCount,
			"_NET_ACTIVE_WINDOW");
	XFree(supportedAtoms);
	return true;
}
...
//			flush_printf("key released...%d\n", event.xkey.keycode);
		...
		// Translate and report key press
		input_key(screen, translate_key(screen, event.xkey.keycode),
		KEY_RELEASE);
	...
	case MotionNotify:
	{
		...
		if (event.xmotion.x != screen->input.CursorPosX
			...
			if (screen->mouse_pos_callback) {
				screen->mouse_pos_callback(screen->input.mousePosX,
						screen->input.mousePosY);
			}
		}
		break;
	}
...
//        }
	...
	case ClientMessage:
	{
		if ((Atom) event.xclient.data.l[0] == screen->wmDeleteWindow) {
			...
			XSendEvent(screen->display, event.xclient.window,
			False,
